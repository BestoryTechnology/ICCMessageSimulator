...
using System;
  ...
  public class IniFile
  {
    ...
    public static string defaultFolderPath = Path.Combine(FSIOHelper.GetCurrentDirectory(), applicationName);
    public static string defaultFilePath
    {
        get
        {
#if UNITY_STANDALONE || UNITY_EDITOR
            return Path.Combine(defaultFolderPath, "AppConfig.ini");
#else
            return Path.Combine(defaultFolderPath, "AppConfig.osc");
#endif
        }
    }// = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), applicationName + "\\AppConfig.ini");
    ...
    {
        return FileExist(defaultFilePath);
    }
    ...
    /// <summary>
    /// Load Ini File From Default Application AppData Folder
    /// </summary>
    public void Load()
    {
        Console.WriteLine("Load Ini From: "+ defaultFilePath);
        Load(defaultFilePath);
    }

    /// <summary>
    /// Save Ini File To Default Application AppData Folder
    /// </summary>
    public void Save()
    {
    ...
    public void WriteClassDataToSection(object obj, string sectionName = null)
    {
        IniSection usingSection = this.GetSection(string.IsNullOrEmpty(sectionName) ? obj.GetType().Name : sectionName);
        usingSection.RemoveAllKeys();

        Type T = obj.GetType();
        foreach (var field in T.GetFields())
        {
            object value = field.GetValue(obj);
            usingSection.AddKey(field.Name).Value = (value == null ? "" : value.ToString());
        }
    }

    public bool LoadClassDataFromSection(object obj, string sectionName = null)
    {
        try
        {
            IniSection usingSection = this.GetSection(string.IsNullOrEmpty(sectionName) ? obj.GetType().Name : sectionName);
            
            if(usingSection == null || usingSection.Keys.Count == 0)
            {
                return false;
            }
            ...
            foreach (var field in T.GetFields())
            {
                string filedValue = usingSection.GetKey(field.Name, "").Value;
                
                
                switch (field.FieldType.Name.ToLower())
                {
                    case "int32":
                        field.SetValue(obj, int.Parse(filedValue));
                        break;

                    case "single":
                        field.SetValue(obj, float.Parse(filedValue));
                        break;

                    case "string":
                        field.SetValue(obj, filedValue);
                        break;

                    case "double":
                        field.SetValue(obj, double.Parse(filedValue));
                        break;

                    case "boolean":
                        field.SetValue(obj, bool.Parse(filedValue));
                        break;

                    case "byte":
                        field.SetValue(obj, byte.Parse(filedValue));
                        break;

                    case "short":
                        field.SetValue(obj, short.Parse(filedValue));
                        break;

                    case "long":
                        field.SetValue(obj, long.Parse(filedValue));
                        break;

                    default:
                        string baseType = field.FieldType.BaseType.Name.ToLower();
                        ...
                        switch(baseType)
                        {
                            case "enum":
                                try
                                {
                                    var parsedValue = Enum.Parse(field.FieldType, filedValue);
                            ...
                            default:
                                Console.WriteLine("Unhandled Variable Type: " + field.FieldType.Name.ToLower());
                                break;
                        }
        ...
        {
            Console.WriteLine("Error While Parsing Data: " + exc.ToString()+"\nStackTrace: "+exc.StackTrace);
            return false;
        }
      ...
      while (!oReader.EndOfStream)
      {
        ...
        if (line != string.Empty)
        {
          ...
          else if (regexsection.Match(line).Success)
          {
            ...
            //Trace.WriteLine(string.Format("Loading section [{0}]", m.Groups[1].Value));
          ...
          else if (regexkey.Match(line).Success && tempsection != null)
          {
            ...
            //Trace.WriteLine(string.Format("Loading Key [{0}]=[{1}]", m.Groups[1].Value, m.Groups[2].Value));
          ...
          else if (tempsection != null)
          {
            ...
            //Trace.WriteLine(string.Format("Loading Key [{0}]", line));
    ...
    public class IniSection
    {
      ...
      /// <param name="key"></param>
      /// <returns>Returns a IniKey object to the key by name, NULL if it was not found</returns>
      public IniKey GetKey(string key, string defaultValue)
      {
          // Trim spaces
          key = key.Trim();
          ...
          {
              AddKey(key).Value = defaultValue;
          }

          return _keys[key] ;
      }

      public void SetKeyValue(string key, string toValue)
      {
          if (!_keys.ContainsKey(key))
          {
              AddKey(key).Value = toValue;
          }
          else
          {
              _keys[key].Value = toValue;
          }
      }
